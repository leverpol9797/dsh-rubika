 ...
 *   RUBIKA_ALLOWED_USERS       — comma-separated user IDs (empty = block ALL users)
 *   RUBIKA_ALLOW_ALL_USERS     — optional, "true" to allow everyone
 *   RUBIKA_GROUP_ALLOWED_CHATS — comma-separated group chat IDs (empty = block ALL groups)
 */
...
async function handleUserMessage(ctx, token, chatId, text, senderId, extraBlocks) {
  // ── Authorization ──
  // If RUBIKA_ALLOW_ALL_USERS is not "true", ALL users are blocked
  // unless they are explicitly listed in RUBIKA_ALLOWED_USERS.
  if (!isAllowAll()) {
...
async function processUpdate(ctx, token, update) {
  ...
  // If RUBIKA_GROUP_ALLOWED_CHATS is not set, ALL groups are blocked.
  // Only groups explicitly listed in the comma-separated env var are allowed.
  // In groups, bot only responds if message contains "هرمس" (trigger word).
  ...
  if (chatInfo.type === "group") {
    ...
    // In allowed groups, only respond if message contains trigger word "هرمس"
    const fullTextForCheck = (msg.text || "").trim();
    if (!fullTextForCheck.includes("هرمس")) {
      return; // silently ignore messages without trigger word in groups
    }
  }
